#!/usr/bin/env python3
"""
GRACE Basin Identification via Rotated EOFs (Masked Batch Generation & Shapefile Export)
Date: 2026
"""
...
# Optional GIS libraries for Shapefile export
try:
    import rioxarray
...
OUTPUT_DIR = "africa_output"            # Folder where all maps/shapefiles will be saved
...
# =====================================================================
# 4. BATCH ITERATION LOOP (PLOT, SAVE ALL MODES & EXPORT SHAPEFILES)
# =====================================================================
...
for mode_id in mode_ids:
    ...
    # -----------------------------------------------------------------
    # SHAPEFILE EXPORT SUB-MODULE
    # -----------------------------------------------------------------
    if HAS_GIS and valid_cells_count > 0:
        try:
            # Set up output path unique to this mode
            shp_output_path = os.path.join(OUTPUT_DIR, f"basin_mode_{mode_id}.shp")
            
            # Ensure spatial coordinate metadata is written properly for rasterio
            grid_anomaly.rio.write_crs("EPSG:4326", inplace=True)
            
            # Fill NaNs with 0 and prep for geometry generation
            ...
            # Extract just the basin geometries (where value == 1)
            basin_polygons = [shape(geom) for geom, val in shapes if val == 1]
            
            if basin_polygons:
                # Store attributes, including the mode index for tracking purposes
                gdf = gpd.GeoDataFrame(
                    {'mode_id': [int(mode_id)] * len(basin_polygons)}, 
                    geometry=basin_polygons, 
                    crs="EPSG:4326"
                )
                gdf.to_file(shp_output_path)
                print(f"      [Export]: Successfully saved vectors to: {shp_output_path}")
        except Exception as e:
            print(f"      [Export Error - Mode {mode_id}]: Failed to generate shapefile. Reason: {e}")
    elif not HAS_GIS:
        print("      [Export Skipped]: Install 'geopandas' and 'rioxarray' to enable shapefile exports.")

    # -----------------------------------------------------------------
    # MAP VISUALIZATION SUB-MODULE
    # -----------------------------------------------------------------
...
print(f"\nExecution Complete! All files are saved inside the '{OUTPUT_DIR}' folder.")